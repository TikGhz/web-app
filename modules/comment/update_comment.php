<?
	...
	$sql="UPDATE comment SET	comment_detail='$_POST[comment_detail]' WHERE comment_id='$_POST[id]'";
		
	if(mysql_query($sql) or die (mysql_error())){
		echo "<script language='javascript'>alert('ระบบได้ทำการอัพเดทข้อมูลเรียบร้อยแล้ว')</script>";
		mysql_close();
		echo "<script type='text/javascript'>window.location='index.php?mo=comment&do=manage_comment';</script>";
	}
	else{
