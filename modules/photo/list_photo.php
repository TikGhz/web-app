...
?>
  <tr>
    ...
    <td align="center" style="background:#f9f9f9">
      ...
      <a href="index.php?mo=photo&do=delete_photo&id=<?=$row['image_id']?>" onclick="return confirm('ลบรูป <?=$row['image_name']?> ?')">
...
echo "<div id='page' style='float:right; font-size:14px;'>";
		...
		if($page_id!=1){
			echo "<a href='index.php?mo=photo&do=manage_photo&page_id=1' class='previous-off'>« หน้าแรก</a> ";
			echo "<a href='index.php?mo=photo&do=manage_photo&page_id=$before_page&rowpage=$_GET[rowpage]' class='previous'>หน้าที่แล้ว</a> ";
		}
			for($i=1;$i<=$all_pages;$i++){
				if($page_id==$i){
					echo "<font class='active'><strong> $i </strong></font>";
				}else{
					echo "<a href='index.php?mo=photo&do=manage_photo&page_id=$i&rowpage=$_GET[rowpage]'> $i </a>  ";
				}
			}
		if($page_id!=$all_pages){
			echo "<a href='index.php?mo=photo&do=manage_photo&page_id=$next_page&rowpage=$_GET[rowpage]' class='next'>หน้าถัดไป</a> ";
			echo "<a href='index.php?mo=photo&do=manage_photo&page_id=$all_pages' class='next-off'>หน้าสุดท้าย »</a> ";
		}
