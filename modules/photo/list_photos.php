<?
	$sql_page="SELECT * FROM image WHERE member_id='$_SESSION[login_id]'";
	...
	<input type='hidden' name='do' value='list_photo'>
...
$sql="SELECT * FROM image WHERE member_id='$_SESSION[login_id]' ORDER BY image_id DESC LIMIT $start_row , $rowperpage";
...
?>
  <tr>
    <td align="center"><?=$row['image_id']?></td>
    <td align="center">
    <? if($row['album_id']==1){ ?>
	...
	echo "<a href='index.php?mo=photo&do=detail&id=$row[image_id]'>";
    ...
    <td align="center" style="background:#f9f9f9">
      <a href="index.php?mo=photo&do=edit_photo&id=<?=$row['image_id']?>">
...
echo "<div id='page' style='float:right; font-size:14px;'>";
		...
		if($page_id!=1){
			echo "<a href='index.php?mo=photo&do=list_photo&page_id=1' class='previous-off'>« หน้าแรก</a> ";
			echo "<a href='index.php?mo=photo&do=list_photo&page_id=$before_page&rowpage=$_GET[rowpage]' class='previous'>หน้าที่แล้ว</a> ";
		}
			for($i=1;$i<=$all_pages;$i++){
				if($page_id==$i){
					echo "<font class='active'><strong> $i </strong></font>";
				}else{
					echo "<a href='index.php?mo=photo&do=list_photo&page_id=$i&rowpage=$_GET[rowpage]'> $i </a>  ";
				}
			}
		if($page_id!=$all_pages){
			echo "<a href='index.php?mo=photo&do=list_photo&page_id=$next_page&rowpage=$_GET[rowpage]' class='next'>หน้าถัดไป</a> ";
			echo "<a href='index.php?mo=photo&do=list_photo&page_id=$all_pages' class='next-off'>หน้าสุดท้าย »</a> ";
		}
...
<table class="data" align="center" cellpadding="10" cellspacing="0" >
  ...
  <tr>
    <td colspan="2"><form action="index.php?mo=photo&do=insert_photo" method="post" enctype="multipart/form-data" name="user">
      <table width="100%" border="0" cellspacing="0" cellpadding="5">
        <tr>
          <td width="100">เลือกร้าน</td>
          <td>
            <? $sql1="SELECT * FROM store WHERE member_id='$_SESSION[login_id]'"; 
        ...
        <tr>
          <td>เลือกรีวิว</td>
          <td>
          <? $sql1="SELECT * FROM review WHERE member_id='$_SESSION[login_id]'"; 
			$result1=mysql_query($sql1) or die(mysql_error());
			?>
        ...
        <tr>
          <td><input type="hidden" name="id" id="album_list" value="1"></td>
