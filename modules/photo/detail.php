...
<?
	...
	$sql_page="SELECT * FROM image WHERE store_id='$_GET[id]'";
...
<?
	echo "<div class='clear'></div>";
		...
		?> 
        <div class="photo-name">ภาพเกี่ยวกับร้าน <? echo $c;?></div>
        <div class="gray-line" style="margin-bottom:10px;"></div> 
		<?
		$sql="SELECT * FROM image WHERE store_id='$_GET[id]' ORDER BY image_id DESC LIMIT $start_row , $rowperpage";
		$result=mysql_query($sql) or die(mysql_error());
...
echo "<div id='page'>";
	echo "<center>";
		...
		if($page_id!=1){
			echo "<a href='index.php?mo=photo&do=detail&id=$_GET[id]&page_id=1' class='previous-off'>« หน้าแรก</a> ";
			echo "<a href='index.php?mo=photo&do=detail&id=$_GET[id]&page_id=$before_page&rowpage=$_GET[rowpage]' class='previous'>หน้าที่แล้ว</a> ";
		}
			for($i=1;$i<=$all_pages;$i++){
				if($page_id==$i){
					echo "<font class='active'><strong> $i </strong></font>";
				}else{
					echo "<a href='index.php?mo=photo&do=detail&id=$_GET[id]&page_id=$i&rowpage=$_GET[rowpage]'> $i </a>  ";
				}
			}
		if($page_id!=$all_pages){
			echo "<a href='index.php?mo=photo&do=detail&id=$_GET[id]&page_id=$next_page&rowpage=$_GET[rowpage]' class='next'>หน้าถัดไป</a> ";
			echo "<a href='index.php?mo=photo&do=detail&id=$_GET[id]&page_id=$all_pages' class='next-off'>หน้าสุดท้าย »</a> ";
		}
